...
class ZipCodeValidator extends ConstraintValidator
{
    ...
    public function validate($zipcode, Constraint $constraint)
    {
        ...
        $iso_code = $this->accessor->getValue($this->context->getObject(), $constraint->iso_property);
        if(array_key_exists($iso_code, $this->patterns)){
            $pattern = $this->patterns[$iso_code];
